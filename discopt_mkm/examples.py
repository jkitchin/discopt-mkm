...
from discopt_mkm import CSTR, DifferentialReactor, Model
...
def co_oxidation(T: float = 500.0):
    ...
    -------
    (model, reactor)
        A :class:`~discopt_mkm.model.MicrokineticModel` and a
        :class:`~discopt_mkm.reactors.DifferentialReactor` at fixed gas
        partial pressures, ready for :func:`~discopt_mkm.solve_steady_state`.
    """
...
def selective_oxidation(T: float = 500.0, P_O2: float = 0.5):
    ...
    :func:`~discopt_mkm.solve_steady_state` (use ``coordinates="log"`` with a
...
def overcomplete_co_oxidation(T: float = 500.0):
    ...
    Used by :mod:`discopt_mkm.select` to demonstrate recovering the minimal
